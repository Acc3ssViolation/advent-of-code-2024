    ...
    internal class Day13_2 : IAssignment
    {
        ...
        public string Run(IReadOnlyList<string> input)
        {
            ...
            for (var i = 0; (i + 2) < input.Count; i += 4)
            {
                ...
                var machine = new ClawMachine(new RealPoint(a[0], a[1]), new RealPoint(b[0], b[1]), new RealPoint(p[0] + 10000000000000, p[1] + 10000000000000));
        ...
        private static long FindMinTokens(ref ClawMachine machine)
        {
            var a = machine.A;
            var b = machine.B;
            var p = machine.Prize;
            ...
            var mInt = (long)Math.Round(m);
            var nInt = (long)Math.Round(n);
            if (Math.Abs(m - mInt) < 0.001 && Math.Abs(n - nInt) < 0.001)
            {
